...
CSV_COLUMNS = [
    ...
    "year", "book_type", "chapter", "page_start", "page_end", "coverage_notes",
]
...
def run(syllabus_path, book_files_path, out_path, subject_filter=None,
    ...
    print(f"\nDone. {len(rows)} rows written -> {out_path}")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="Map syllabus topics to textbook page ranges.")
    ap.add_argument("--syllabus", required=True, help="Path to syllabus PDF")
    ap.add_argument("--book-files", required=True, help="Path to book_files.json")
    ap.add_argument("--out", default="mapped_topics.csv", help="Output CSV path")
    ap.add_argument("--subjects", nargs="*", default=None,
                     help="Subject codes to process (default: all subjects found)")
    ap.add_argument("--backend", choices=["gemini", "ollama", "openai", "omniroute"], default="gemini")
